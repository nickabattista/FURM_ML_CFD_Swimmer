...
function [X,Y,U,V,xLag,yLag,C] = IBM_Driver_SWIMMER(Fluid_Params,Grid_Params,Time_Params,Lag_Struct_Params,Output_Params,Lag_Name_Params,Con_Params)

%-------------------------------------------------------------
% READ IN x/y point positions to interpolate btwn
%           xPos(i,j): i - lagrangian index 
%                      j - which phase 
%
%   ---> note: update_NonInv_Beams.m
%   ---> note: update_Target_Point_Positions.m
%   ---> different force spread from Lags onto Fluid Grid
%           to account for no x-Forces by target points
%-------------------------------------------------------------
num_Kinematics_Store = 250; % # of PHASES saved in one full cycle (period)
xPos = read_In_Positions('swimmer_X.phases',num_Kinematics_Store);
yPos = read_In_Positions('swimmer_Y.phases',num_Kinematics_Store);
%
freq = 5;   % undulation wave frequency
period = 1/freq;            % anguilliform wave period
timeVec = linspace(0,period,num_Kinematics_Store);  % partition each undulation period
DeltaT = timeVec(2)-timeVec(1);                     % length of each partition
...
if restart_Flag == 0
    ...
    Fxh = vort; Fyh = vort; F_Lag = zeros( Nb, 2); uLag = zeros(Nb,1); vLag = uLag;
    print_vtk_files(Output_Params,0,ctsave,...
                    vort,uMag,p,U',V',Lx,Ly,Nx,Ny,lagPts,...
                    springs_Yes,connectsMat,tracers,...
                    concentration_Yes,C,Fxh,Fyh,F_Lag,...
                    coagulation_Yes,aggregate_list,...
                    d_Springs_Yes,dconnectsMat,...
                    poroelastic_Yes,F_Poro,...
                    uLag,vLag);
...
while current_time < T_FINAL
    ...
    if ( ( update_Springs_Flag == 1 ) && ( springs_Yes == 1 ) )
       ...
       %------------------------------------------------------------------
       % MODIFIED FOR SWIMMER: for body undulation to keep points
       %                                   certain distances away
       %------------------------------------------------------------------        
       springs_info = update_Springs(dt,current_time,xLag,yLag,springs_info,xPos,yPos,timeVec,DeltaT);
    ...
    if ( ( update_Target_Pts == 1 ) && ( target_pts_Yes == 1) )
       ...
       %------------------------------------------------------------------
       % For SWIMMER 
       %            --> doesn't pass xPos in for SWIMMER model
       %------------------------------------------------------------------
       target_info = update_Target_Point_Positions(dt,current_time,target_info,yPos,timeVec,DeltaT); 
    ...
    if ( ( update_nonInv_Beams_Flag == 1 ) && ( nonInv_beams_Yes == 1) )
       ...
       % For SWIMMER
       %------------------------------------------------------------------
       nonInv_beams_info = update_nonInv_Beams(dt,current_time,nonInv_beams_info,xPos,yPos,timeVec,DeltaT); 
    ...
    %----------------------------------------------------------------------
    % MODIFIED: --> CALCULATES TARGET POINT FORCES IN Y-DIRECTION ONLY!
    %----------------------------------------------------------------------
    [Fxh, Fyh, F_Mass_Bnd, F_Lag, F_Poro, aggregate_list] = please_Find_Lag_Forces_On_Fluid_Grid_SWIMMER(dt, current_time, xLag_h, yLag_h, xLag_P, yLag_P, x, y, grid_Info, Lag_Struct_Params, springs_info, target_info, beams_info, nonInv_beams_info ,muscles_info, muscles3_info, mass_info, electro_potential, d_springs_info, gen_force_info, poroelastic_info, coagulation_info, aggregate_list, flag_Geo_Connect, geo_Connect_MAT);
    ...
    %----------------------------------------------------------------------
    % Add artificial force from fluid boundary, if selected. 
    %----------------------------------------------------------------------
    if arb_ext_force_Yes == 1 
    ...
    %---------------------------------------------------------
    % NOT NEEDED UNLESS FOR A MUSCLE MODEL/DAMPED SPRINGS!
    %---------------------------------------------------------
    xLag_P = []; yLag_P = [];
    ...
    [xLag, yLag, uLag, vLag] = please_Move_Lagrangian_Point_Positions_SWIMMER(mu, Uh, Vh, xLag, yLag, xLag_h, yLag_h, x, y, dt, grid_Info,porous_Yes,poroelastic_Yes,poroelastic_info,F_Poro);
    ...
    if ( ( mod(cter,pDump) == 0  ) && ( cter >= pDump ) )
        ...
        %------------------------------------------------
        % Print data to .vtk files!
        %------------------------------------------------
        lagPts = [xLag yLag zeros(length(xLag),1)];
        print_vtk_files(Output_Params,current_time,ctsave,...
                        vort,uMag',p',U',V',Lx,Ly,Nx,Ny,lagPts,...
                        springs_Yes,connectsMat,tracers,...
                        concentration_Yes,C,Fxh',Fyh',F_Lag,...
                        coagulation_Yes,aggregate_list,...
                        d_Springs_Yes,dconnectsMat,...
                        poroelastic_Yes,F_Poro,uLag,vLag);
...
function print_vtk_files(Output_Params,current_time,ctsave,...
                        vort,uMag,p,U,V,Lx,Ly,Nx,Ny,...
                        lagPts,springs_Yes,connectsMat,tracers,...
                        concentration_Yes,C,fXGrid,fYGrid,...
                        F_Lag,coag_Yes,aggregate_list,...
                        d_springs_Yes,dconnectsMat,...
                        poroelastic_Yes,F_Poro,...
                        uLag, vLag)
...
cd('viz_IB2d'); %Go into viz_IB2d directory
    ...
    %------------------------------------------------------------
    % STORE LAG FORCE INFO TO MAT
    %------------------------------------------------------------
    save(['LAG_INFO.' num2str(ctsave) '.mat'],'uLag','vLag','F_Lag','lagPts');

    %------------------------------------------------------------
    % Save Lagrangian Pts to .vtk format
    %------------------------------------------------------------
    ...
    %------------------------------------------------------------
    % Save DAMP Spring Connections (* if damped springs *)
    %------------------------------------------------------------
    if d_springs_Yes == 1 
        %Print Lagrangian Pts w/ CONNECTIONS to .vtk format
    ...
    %------------------------------------------------------------------
    %                   SAVE (VECTOR) EULERIAN DATA 
    %------------------------------------------------------------------
        
        % Save VELOCITY FIELD to .vtk file
...
%---------------------------------------------   
% Go up a directory (leave viz_IB2d folder)
%--------------------------------------------- 
cd ../

    %------------------------------------------------------------------
    %                SAVE LAGRANGIAN (position,force) DATA 
    %------------------------------------------------------------------
    if Output_Params(18) == 1
        if length( lagPts ) <= 5
            ...
            %------------------------------------------------------------------
            % Save Lagrangian Force/Velocity Data to hier_IB2d_data/ folder
            %------------------------------------------------------------------
            %
            cd('hier_IB2d_data'); %change directory to hier-data folder

                % Save x-y bodyspeed data! 
                % (From velocity interpolation of fluid -> Lagrangian structure)
                uLag_XName = ['uLag.' strNUM '.vtk'];
                vLag_YName = ['vLag.' strNUM '.vtk'];
                savevtk_points_with_scalar_data( lagPts, uLag, uLag_XName, 'uLag', current_time);
                savevtk_points_with_scalar_data( lagPts, vLag, vLag_YName, 'vLag', current_time);
            ...
            cd ../ % Leave hier_IB2d_data folder
        end
    end % ENDS IF-STATEMENT FOR IF SAVING LAGRANGIAN FORCE/VELOCITY DATA
...
%
%
%             ADDED FOR READING IN STATE PHASE DATA INTERPOLATION
% 
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% FUNCTION: Reads in all of the LAGRANGIAN PT POSITIONS in each of the PHASES  
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function PTS = read_In_Positions(struct_name,numStore)
...
% Read in the file, use 'CollectOutput' to gather all similar data together
% and 'CommentStyle' to to end and be able to skip lines in file.
C = textscan(fileID,strVec,'CollectOutput',1);
