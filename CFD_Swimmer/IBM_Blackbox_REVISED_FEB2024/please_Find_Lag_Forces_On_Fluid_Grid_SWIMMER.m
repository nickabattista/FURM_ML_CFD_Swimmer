...
elseif ( ( muscle_LT_FV_Yes == 1 ) && ( electro_phys_Yes == 1 ) )
    ...
    fx_muscles = 0;%zeros(length(xLag),1);
...
if ( muscle_3_Hill_Yes == 1)
    ...
    fx_muscles3 = 0;%zeros(length(xLag),1);
...
if ( springs_Yes == 1 )
    ...
    fx_springs = 0;%zeros(Nb,1); %No x-forces coming from springs
...
if ( mass_Yes == 1)
    ...
    fx_mass = 0;%zeros(Nb,1); %No x-forces coming from mass points
...
if ( target_pts_Yes == 1)
    ...
    fx_target = 0;%zeros(Nb,1); %No x-forces coming from target points
...
if ( beams_Yes == 1 )
    ...
    fx_beams = 0;%zeros(Nb,1); %No x-forces coming from beams
...
if ( nonInv_beams_Yes == 1 )
    ...
    fx_nonInv_beams = 0;%zeros(Nb,1);        %No x-forces coming from beams
...
if ( d_Springs_Yes == 1 )
    ...
    fx_dSprings = 0;%zeros(Nb,1);    %No x-forces coming from damped springs
...
if ( gen_force_Yes == 1 )
    ...
    fx_genForce = 0;%zeros(Nb,1);    %No x-forces coming from general force model
...
if ( coagulation_Yes == 1 )
    ...
    fx_coag = 0;%zeros(Nb,1);   % No x-forces coming from coagulation
...
% SET xTargetForces to 0.
fx_target = zeros(length(xLag),1);  % Initialize storage for x-force density from TARGET PTS
fy_target = fx_target;              % Initialize storage for y-force density from TARGET PTS
...
for i=1:N_targets
    ...
    %fx_target(IDs(i),1) = fx(IDs(i),1) + kStiffs(i)*( dx );
    fy_target(IDs(i),1) = fy_target(IDs(i),1) + kStiffs(i)*( dy ); 
   
end

% DEFINED fx_target earlier to be zero
%fx_target = fx;

% DEFINE fy_target force vector
%fy_target = fy;
