%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% FUNCTION: performs GRADIENT DESCENT to find the best fit cubic 
...
function Gradient_Descent_Best_Fit()
...
%------------------------------------------------------------
coeffsMAT = zeros(NTimes,4); 
lossVec = zeros(NTimes,1);
...
for n=1:NTimes
    ...
    %---------------------------------------------------------
    %        Get next coefficient guess (actual GD step)
    %---------------------------------------------------------
    c0_Next = coeffsPrev(1) - alpha * GRAD( coeffsPrev, XY_Data, 0); % "Partial loss, partial c0"
    c1_Next = coeffsPrev(2) - alpha * GRAD( coeffsPrev, XY_Data, 1); % "Partial loss, partial c1"
    c2_Next = coeffsPrev(3) - alpha * GRAD( coeffsPrev, XY_Data, 2); % "Partial loss, partial c2"
    c3_Next = coeffsPrev(4) - alpha * GRAD( coeffsPrev, XY_Data, 3); % "Partial loss, partial c3"
    %
...
%------------------------------------------------
fprintf('\n---------------------------------------------------\n\n');
fprintf('GRAD DESCENT INFO (epoch, loss, coefficients):\n\n');
fprintf('n | loss | (c_0^n,c_1^n,c_2^n,c_3^n) \n')
for n=1:1000:length(coeffsMAT(:,1))
...
%------------------------------------------------
f1=figure(1);
loglog(0:NTimes,lossVec,'.-','LineWidth',lw,'MarkerSize',ms); hold on;
